...
def rcov_verify
  require File.expand_path(File.dirname(__FILE__) + "/../../utils/coverage_checker")
  coverage_checker = CoverageChecker.new
  if !coverage_checker.all_covered?
    puts "Bad Boy! Coverage is not 100%..."
    puts "Files not fully covered:"
    coverage_checker.not_fully_covered.each do |line|
      puts "#{line[:file]}: #{line[:coverage]}"
    end
    fail
  else
...
namespace :test do
  ...
  namespace :rcov do
    ...
    namespace :functionals do
      ...
      task :verify do
        rcov_verify
        Rake::Task['spec:clobber_rcov'].invoke
      end
    end
  end
